...
import XbtsioRecentTransactions from "components/DepositWithdraw/xbtsio/XbtsioRecentTransactions";

import XbtsioGatewayRequest from "components/DepositWithdraw/xbtsio/XbtsioGatewayRequest";
import LoadingIndicator from "components/LoadingIndicator";
import {xbtsioAPIs} from "api/apiConfig";

class XbtsioGateway extends React.Component {
    constructor(props) {
        super();
    ...
    _getActiveCoin(props, state) {
        let cachedCoin = props.viewSettings.get(
            `activeCoin_XBTSIO_${state.action}`,
            null
        );
    ...
    _getCoins() {
        Promise.all([
            fetchCoins(xbtsioAPIs.BASE + xbtsioAPIs.COINS_LIST),
            fetchTradingPairs(xbtsioAPIs.BASE + xbtsioAPIs.TRADING_PAIRS),
            getActiveWallets(xbtsioAPIs.BASE + xbtsioAPIs.ACTIVE_WALLETS)
        ]).then(result => {
                    ...
                    <div>
                        <div style={{marginBottom: 15}}>
                            <XbtsioGatewayRequest
                                ...
                                min_deposit_amount={coin.minDepositAmount}
                                fee_type={coin.feeType}
                                action={this.state.action}
                            />
                        </div>

                        <XbtsioRecentTransactions
